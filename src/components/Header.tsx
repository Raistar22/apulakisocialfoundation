      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-card shadow-sm">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-16">
              ...
              {navItems.map((item) =>
                item.hasDropdown ? (
                  <HoverCard key={item.name} openDelay={0} closeDelay={100}>
                    <HoverCardTrigger className="flex items-center gap-1 text-sm font-medium text-foreground hover:text-primary transition-colors outline-none cursor-pointer">
                      {item.name}
                      <ChevronDown className="w-4 h-4" />
                    </HoverCardTrigger>
                    <HoverCardContent className="w-48 p-2 bg-card border border-border shadow-lg z-50">
                      <div className="space-y-1">
                        {item.items?.map((subItem) => (
                          <a
                            key={subItem.label}
                            href={subItem.href}
                            className="block px-3 py-2 text-sm text-foreground rounded-md hover:bg-muted transition-colors"
                          >
                            {subItem.label}
                          </a>
                        ))}
            ...
            {/* Donate Button */}
            <Button
              className="hidden md:flex bg-secondary hover:bg-secondary/90 text-secondary-foreground font-medium"
